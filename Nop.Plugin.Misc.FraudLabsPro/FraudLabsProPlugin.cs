...
namespace Nop.Plugin.Misc.FraudLabsPro
{
    ...
    public class FraudLabsProPlugin : BasePlugin, IMiscPlugin, IWidgetPlugin
    {
        ...
        {
            //settings
            await _settingService.SaveSettingAsync(new FraudLabsProSettings()
            {
        ...
        {
            //settings
            await _settingService.DeleteSettingAsync<FraudLabsProSettings>();

            //generic attributes
            /*var orders = _orderService.SearchOrdersAsync();
            foreach (var order in orders)
            {
                var genericAttributes = _genericAttributeService.GetAttributesForEntityAsync(order.Id, order.GetType().Name).ToList()
                ...
                if (genericAttributes.Any())
                    _genericAttributeService.DeleteAttributesAsync(genericAttributes);
            }*/

            //locales
            await _localizationService.DeleteLocaleResourcesAsync("Plugins.Misc.FraudLabsPro");
